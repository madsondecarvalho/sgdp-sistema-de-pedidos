...
class PedidoService {
    ...
    async addItemToPedido(fastify: FastifyInstance, id: string, item: Omit<PedidoItem, 'id_pedido'>) {
        try {
            // Verificar se o pedido existe
            const existingPedido = await this.findById(fastify, id);
            if (!existingPedido) return null;
            ...
            await (fastify as any).mysql.query(
                'INSERT INTO pedido_itens (id_pedido, id_produto, qtde, preco) VALUES (?, ?, ?, ?)',
                [id, item.id_produto, item.qtde, item.preco]
            );

            // Retornar o pedido atualizado
            return this.findById(fastify, id);
        } catch (error) {
            console.error(`Error in addItemToPedido (${id}):`, error);
            throw error;
        }
    }

    // Atualizar um item de pedido
    async updateItem(fastify: FastifyInstance, id_pedido: string, id_produto: string, item: Partial<Omit<PedidoItem, 'id_pedido' | 'id_produto'>>) {
        try {
            // Verificar se o pedido existe
            const existingPedido = await this.findById(fastify, id_pedido);
            if (!existingPedido) return null;
            ...
            if (item.qtde !== undefined) {
                updateParts.push('qtde = ?');
                values.push(item.qtde);
            }
            ...
            // Adiciona os IDs como últimos parâmetros
            values.push(id_pedido);
            values.push(id_produto);

            // Executa a query de atualização
            await (fastify as any).mysql.query(
                `UPDATE pedido_itens SET ${updateParts.join(', ')} WHERE id_pedido = ? AND id_produto = ?`,
                values
            );

            // Retorna o pedido atualizado
            return this.findById(fastify, id_pedido);
        } catch (error) {
            console.error(`Error in updateItem (${id_pedido}, ${id_produto}):`, error);
            throw error;
        }
    }

    // Remover um item de pedido
    async removeItem(fastify: FastifyInstance, id_pedido: string, id_produto: string) {
        try {
            // Verificar se o pedido existe
            const existingPedido = await this.findById(fastify, id_pedido);
            if (!existingPedido) return null;
            ...
            // Remover o item
            await (fastify as any).mysql.query(
                'DELETE FROM pedido_itens WHERE id_pedido = ? AND id_produto = ?',
            ...
            console.error(`Error in removeItem (${id_pedido}, ${id_produto}):`, error);
            throw error;
        }
    }
