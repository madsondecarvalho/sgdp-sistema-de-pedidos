<template>
  ...
  >
    <v-card rounded="lg">
      <v-toolbar color="error" density="compact" class="pl-4">
...
const emit = defineEmits(['update:modelValue', 'cliente-excluido', 'error']);
...
const excluirCliente = async () => {
  try {
    ...
    await axios.delete(`http://localhost:3333/clientes/${props.cliente.id}`);

    // Emitir evento de sucesso
    emit('cliente-excluido', props.cliente.id);
